...
test-coverage:
	PYTHONPATH=. poetry run pytest tests --cov production_rag --cov-report term --cov-report=html --cov-report xml --junit-xml=tests-results.xml

black:
	poetry run black . --check

ruff:
	poetry run ruff check production_rag tests

format:
	poetry run black .
	poetry run ruff check production_rag tests --fix

mypy:
	poetry run mypy production_rag
...
dev-windows:
	(set PGPT_PROFILES=local & poetry run python -m uvicorn production_rag.main:app --reload --port 8001)

dev:
	PYTHONUNBUFFERED=1 PGPT_PROFILES=local poetry run python -m uvicorn production_rag.main:app --reload --port 8001
...
api-docs:
	PGPT_PROFILES=mock poetry run python scripts/extract_openapi.py production_rag.main:app --out fern/openapi/openapi.json
